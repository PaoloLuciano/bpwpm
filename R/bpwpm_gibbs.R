...
#' Polinomial model described in the thesis.
#'
...
#' @param precision If using the default sigmas for betas and w, a diagonal
...
#' @export
#'
#' @examples See test for more detailed examples
bpwpm_gibbs <- function(Y, X, M, J, K,
                  intercept = TRUE, precision = 1,
                  draws = 10^3,
    ...
    if(intercept){
        ...
        if(is.null(sigma_beta_0_inv)){
            sigma_beta_0_inv <- precision*diag(d+1)
        }
    }else{
        if(is.null(beta_init)){
        ...
        if(is.null(sigma_beta_0_inv)){
            sigma_beta_0_inv <- precision*diag(d)
        }
    }
    ...
    if(is.null(w_init)){
        w <- matrix(1/N, nrow = N, ncol = d) # Standard beta
    }else{
        w <- w_init
    }
    ...
    if(is.null(sigma_w_0_inv)){
        sigma_w_0_inv <- rep(list(precision*diag(N)),d)
    }
    ...
    for(k in seq(1,draws)){
        ...
        for(j in 1:d){
            ...
            if(debug){cat("\n\tPartial Residuales:", format(head(h),digits = 2, width = 10), cat = "")}
